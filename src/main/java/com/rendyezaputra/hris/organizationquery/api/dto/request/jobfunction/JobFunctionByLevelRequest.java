...
@AllArgsConstructor
@Builder
public class JobFunctionByLevelRequest {
    @Min(0) @Max(32767)
    short level;
    ...
    @ValueOfEnum(enumClass = ComparisonOperator.class, message = "{validation.operator.message}")
    private String operator = ComparisonOperator.EQ.name();

    @ValueOfEnum(enumClass = JobFunctionSortField.class, message = "{validation.sortBy.message}")
    private String sortBy = "level";
